...
def gen_ds(dataset_name):
    print(f"{datetime.now()} generate dataset:{dataset_name}")
...
def gen_sh_mx(dataset_name, num_nodes, adj_mx):
    print(f"{datetime.now()} generate sh mx:{dataset_name}")
    sd_mx = None
    type_short_path = 'hop'
...
def gen_dtw(dataset_name, num_nodes, df):
    print(f"{datetime.now()} generate dtw:{dataset_name}")
    points_per_hour = 12
...
def gen_patterns(dataset_name, x_train):
    print(f"{datetime.now()} generate patterns beginning:{dataset_name}")
    points_per_day = 288
    output_dim = 1
    ...
    if not os.path.exists(pattern_key_file + '.npy'):
        ...
        print(f"{datetime.now()} generate patterns finish:{dataset_name}")
    else:
