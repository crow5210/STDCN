...
def generate_data(args):
    ...
    dtw_file_path = args.get("DTW_FILE_PATH",None)
    shmx_file_path = args.get("SHMX_FILE_PATH",None)
    pattern_file_path = args.get("PATTERNKESY_FILE_PATH",None)
    ...
    if dtw_file_path is not None and os.path.exists(dtw_file_path):
        shutil.copyfile(dtw_file_path, output_dir + "/dtw_mx.npy")

    if shmx_file_path is not None and os.path.exists(shmx_file_path):
        shutil.copyfile(shmx_file_path, output_dir + "/sh_mx.npy")

    if pattern_file_path is not None and os.path.exists(pattern_file_path):
        shutil.copyfile(pattern_file_path, output_dir + "/pattern_mx.npy")
...
def build_dataset(cfg_general,cfg_dataset):
    ...
    datasets['null_val'] = cfg_dataset.NULL_VAL
    datasets['mask_val'] = cfg_dataset.MASK_VAL
    return datasets
