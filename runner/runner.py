...
from builder import build_model,validate,fit,inference
...
from utils.gen_report import result_decode,gen_ranking_graph,gen_polar_graph
import torch
...
def visualize_result(algos,epochs,out_dir,device,data = "Electricity"):
    ...
    for algo in algos:
        ...
        if checkpoint_dict is not None:            
            ...
            with torch.no_grad():
                ...
                hist.append(torch.cat([history_data[0,:,1,0].detach().cpu(),torch.tensor([torch.nan]*len_future)]))
                pred.append(torch.cat([torch.tensor([torch.nan]*len_history),prediction_data[0,:,1,0].detach().cpu()]))
                futu.append(torch.cat([torch.tensor([torch.nan]*len_history),future_data[0,:,1,0].detach().cpu()]))
    ...
    fig.update_layout(title="visualize prediction value", 
                font_family="San Serif",
                bargap=0.2,
                titlefont={'size': 24},
                height=200*len(algos),
                width=800  
                )
...
def train_test(datas,algos,epochs,val_interval,out_dir,device):
    ...
    for algo in algos:
        for data in datas:
            ...
            if checkpoint_dict is not None:            
                models["model"].load_state_dict(checkpoint_dict['model_state_dict'], strict=True)
                result_total[f"{data}_{algo}"] = inference(models, datasets, CFG_GENERAL.METRICS, CFG_GENERAL.TEST.get("HORIZON",None), device, logger, ckpt_save_dir)
            else:
                logger.info(f"can't find ckpt:{ckpt_path}")

            tensbd.close()

    report_df = result_decode(result_total,out_dir)
