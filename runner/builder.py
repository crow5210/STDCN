...
def epoch_iter(models,datasets,epoch,mode,metrics,horizons,logger,tensbd,device, out_dir=None):
    ...
    for (future_data, history_data) in tqdm(data_loader):
        ...
        if mode=="train":
            ...
            loss = models["loss"](prediction,real_value, datasets['null_val'], datasets['mask_val'])
            optimizer.zero_grad()
            loss.backward()
            if clip_grad_param:
        ...
        for metric_name, metric_func in metrics.items():
            if mode=="test" and isinstance(horizons,list):
                for h in horizons:
                    metric_item = metric_func(*[prediction[:,h-1,:,:], real_value[:,h-1,:,:], datasets['null_val'], datasets['mask_val']])
