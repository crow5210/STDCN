...
from baselines.PDFormer import loss
...
class PDFormer(nn.Module):
    def __init__(self, **model_args):
        ...
        self._scaler = model_args.get('scaler')
        self.num_nodes = model_args.get("num_nodes", 1)
        self.feature_dim = model_args.get("feature_dim", 1)
        self.ext_dim = model_args.get("ext_dim", 0)
        # self.num_batches = model_args.get('num_batches', 1)
        self.dtw_matrix = model_args.get('dtw_matrix')
        self.adj_mx = model_args.get('adj_mx')
        sd_mx = model_args.get('sd_mx')
        sh_mx = model_args.get('sh_mx')
        self._logger = getLogger()
        self.dataset = model_args.get('dataset')

        self.embed_dim = model_args.get('embed_dim', 64)
        self.skip_dim = model_args.get("skip_dim", 256)
        lape_dim = model_args.get('lape_dim', 8)
        geo_num_heads = model_args.get('geo_num_heads', 4)
        sem_num_heads = model_args.get('sem_num_heads', 2)
        t_num_heads = model_args.get('t_num_heads', 2)
        mlp_ratio = model_args.get("mlp_ratio", 4)
        qkv_bias = model_args.get("qkv_bias", True)
        drop = model_args.get("drop", 0.)
        attn_drop = model_args.get("attn_drop", 0.)
        drop_path = model_args.get("drop_path", 0.3)
        self.s_attn_size = model_args.get("s_attn_size", 3)
        self.t_attn_size = model_args.get("t_attn_size", 3)
        enc_depth = model_args.get("enc_depth", 6)
        type_ln = model_args.get("type_ln", "pre")
        self.type_short_path = model_args.get("type_short_path", "hop")

        self.output_dim = model_args.get('output_dim', 1)
        self.input_window = model_args.get("input_window", 12)
        self.output_window = model_args.get('output_window', 12)
        add_time_in_day = model_args.get("add_time_in_day", True)
        add_day_in_week = model_args.get("add_day_in_week", True)
        self.device = model_args.get('device', torch.device('cpu'))
        self.world_size = model_args.get('world_size', 1)
        self.huber_delta = model_args.get('huber_delta', 1)
        self.quan_delta = model_args.get('quan_delta', 0.25)
        self.far_mask_delta = model_args.get('far_mask_delta', 5)
        self.dtw_delta = model_args.get('dtw_delta', 5)

        self.use_curriculum_learning = model_args.get('use_curriculum_learning', True)
        self.step_size = model_args.get('step_size', 2500)
        self.max_epoch = model_args.get('max_epoch', 200)
        self.task_level = model_args.get('task_level', 0)
        ...
        self.batch_lap_pos_enc = _cal_lape(self.adj_mx).to(self.device)
        ...
        self.pattern_keys = torch.from_numpy(model_args.get('pattern_keys')).float().to(self.device)
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool, **kwargs):
        return self.PDFormerForward({"X":history_data},self.batch_lap_pos_enc)

    def PDFormerForward(self, batch, lap_mx=None):
        x = batch['X']
        T =  x.shape[1]
        x_pattern_list = []
